...
import { Cloud, Lock, Mail, User, Eye, EyeOff, ArrowRight, AlertCircle, Loader2, Check } from 'lucide-react';
...
export const SignUpPage: React.FC<SignUpPageProps> = ({ onNavigate }) => {
  const { signUpWithEmail, loginWithGoogle, authError, clearAuthError } = useAuth();
  ...
  return (
    <div id="signup-page" className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <p className="mt-2 text-xs sm:text-sm text-slate-600 dark:text-slate-400">
          Join thousands of developers and professionals organizing knowledge in the cloud
        </p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center gap-2 pt-1">
              <input
                ...
                className="w-4 h-4 text-indigo-600 rounded-sm border-slate-300 dark:border-slate-700 focus:ring-indigo-500"
              />
              <label htmlFor="signup-terms-check" className="text-xs text-slate-600 dark:text-slate-400">
